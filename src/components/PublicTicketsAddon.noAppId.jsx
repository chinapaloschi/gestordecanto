...
export default function PublicTicketsSection({
  ...
  React.useEffect(() => {
    ...
    // No exigimos appId en el doc para que funcione con datos existentes.
    const q = query(collectionGroup(db, 'tickets'), where('studentId', '==', student.id));

    const unsub = onSnapshot(q, (snap) => {
      const rows = snap.docs.map(d => ({ id: d.id, ...d.data(), __path: d.ref.path }));
      // Filtro defensivo: solo tickets bajo /artifacts/{appId}/
      const safe = rows.filter(r => r.__path.includes(`/artifacts/${appId}/`));
      setTickets(safe);
    }, (e) => { console.error("PublicTicketsSection tickets:", e); setTickets([]); });
  ...
  return (
    <div className="mt-6">
      <div className="text-sm font-semibold text-gray-800 mb-2">Entradas emitidas para este alumno</div>
