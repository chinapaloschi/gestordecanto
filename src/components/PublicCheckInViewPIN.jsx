...
const getCurrentWeekRange = () => {
  ...
  // Repertorio asignado por el docente
  React.useEffect(() => {
    ...
    const unsub = onSnapshot(qRep, snap => setRepertoire(snap.docs.map(d => ({ id: d.id, ...d.data() }))), () => {});
    return unsub;
  }, [db, appId, student?.id]);
    ...
    (async () => {
      try {
        ...
        const rows = snap.docs
          ...
          .filter(c => (c.classNotes && c.classNotes.trim()) || (c.topicLink && c.topicLink.length > 5));
        if (!cancelled) setClassNotesList(rows);
      } catch {}
    })();
      ...
      {student && student.photoURL && (
        <div className="min-h-screen bg-gray-50 font-body text-gray-900 flex flex-col">
          ...
          {activeTab === 'inicio' && (() => {
            ...
            const showCta = publicTotalToday > 0;
            const showNext = !!next;
          ...
          {activeTab === 'clases' && <div className="space-y-3">
            ...
            {classNotesList.length > 0 && (
              <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
                <button onClick={() => setOpenNotes(p => !p)}
                  className="w-full flex items-center justify-between px-4 py-3.5 hover:bg-gray-50 transition text-left">
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-full bg-rose-50 flex items-center justify-center flex-shrink-0">
                      <svg className="w-4 h-4 text-rose-500" fill="none" stroke="currentColor" strokeWidth={1.8} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                    </div>
                    <span className="font-display font-semibold text-sm text-gray-800">Material y notas de clase</span>
                  </div>
                  <svg className={`w-4 h-4 text-gray-300 transition-transform ${openNotes ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7"/></svg>
                </button>
                {openNotes && (
                  <div className="divide-y divide-gray-50 max-h-72 overflow-y-auto">
                    {classNotesList.map(cls => (
                      <div key={cls.id} className="px-4 py-3">
                        <p className="font-ticket text-[10px] text-gray-400 uppercase tracking-wide mb-1">
                          {formatDateToDDMMYYYY(new Date(cls.classDate + 'T12:00:00'))} · {cls.startTime || cls.classTime || ''}
                        </p>
                        {cls.classNotes && cls.classNotes.trim() && (
                          <p className="text-sm text-gray-700 leading-snug whitespace-pre-wrap mb-1.5">{cls.classNotes}</p>
                        )}
                        {cls.topicLink && cls.topicLink.length > 5 && (
            ...
            {repertoire.length > 0 && (() => {
              ...
              return (
                <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
                  <button onClick={() => setOpenRepertoire(p => !p)}
                    className="w-full flex items-center justify-between px-4 py-3.5 hover:bg-gray-50 transition text-left">
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-full bg-rose-50 flex items-center justify-center flex-shrink-0">
                        <svg className="w-4 h-4 text-rose-500" fill="none" stroke="currentColor" strokeWidth={1.6} viewBox="0 0 24 24"><circle cx="9" cy="18" r="3"/><path strokeLinecap="round" strokeLinejoin="round" d="M12 18V4l7 2v12"/></svg>
                      </div>
                      <span className="font-display font-semibold text-sm text-gray-800">Mi repertorio</span>
                      <span className="text-[11px] font-semibold text-gray-300">{repertoire.length}</span>
                    </div>
                    <svg className={`w-4 h-4 text-gray-300 transition-transform ${openRepertoire ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7"/></svg>
                  </button>
                  {openRepertoire && (
