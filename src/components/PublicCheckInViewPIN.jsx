    ...
    <>
      ...
      {student && student.photoURL && (
        <div className="min-h-screen bg-gray-50 flex flex-col">
          ...
          {/* Header compacto */}
          <div className="bg-gradient-to-r from-rose-700 to-pink-600 px-4 pb-4 flex items-center gap-3 flex-shrink-0"
            ...
            <div className="flex-1 min-w-0">
              <p className="text-white font-black text-base uppercase tracking-tight truncate leading-tight">{student.name}</p>
              <p className="text-rose-200 text-[11px] leading-tight">{hoyStr}</p>
            </div>
            {hasPaidThisMonth
              ? <span className="text-[10px] font-bold text-green-100 bg-green-500/30 border border-green-300/30 px-2 py-1 rounded-full whitespace-nowrap">✅ Al día</span>
              : publicTotalToday > 0
                ? <span className="text-[10px] font-bold text-white bg-white/20 border border-white/30 px-2 py-1 rounded-full whitespace-nowrap animate-pulse">💳 Debe</span>
                : null}
            <button onClick={cambiarUsuario}
              className="text-[11px] font-semibold text-rose-200 hover:text-white transition whitespace-nowrap flex-shrink-0">
              Cambiar usuario
            </button>
          </div>
            ...
            return <div className="space-y-3">
              ...
              {showScanCta && (
                <div className="space-y-1.5">
                  <a href={`${window.location.origin}/escanear-presente.html`}
                    ...
                    className="w-full block rounded-2xl p-5 shadow-md bg-gradient-to-br from-green-600 to-emerald-500 text-left active:scale-[0.98] transition">
                    <div className="flex items-center gap-3">
                      <div className="w-12 h-12 rounded-2xl bg-white/20 flex items-center justify-center text-2xl flex-shrink-0">📷</div>
                      <div className="flex-1 min-w-0">
                        <p className="text-white font-black text-sm">Escanear QR para dar presente</p>
              ...
              {showPaid && (
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 flex items-center gap-3">
                  ...
                  <div><p className="font-bold text-green-800 text-sm">¡Cuota al día!</p><p className="text-xs text-green-600">Tu pago de este mes fue recibido. ¡Gracias!</p></div>
                </div>
              )}
              {showDue && (
                <div className={`rounded-2xl p-4 shadow-sm flex items-center gap-3 ${aplicaRecargo ? 'bg-red-50 border border-red-200' : 'bg-amber-50 border border-amber-200'}`}>
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 text-xl ${aplicaRecargo ? 'bg-red-100' : 'bg-amber-100'}`}>{aplicaRecargo ? '🔴' : '🟡'}</div>
                  <div>
                    <p className={`font-bold text-sm ${aplicaRecargo ? 'text-red-800' : 'text-amber-800'}`}>{aplicaRecargo ? 'Cuota vencida — 10% de recargo' : 'Cuota pendiente'}</p>
                    <p className={`text-xs ${aplicaRecargo ? 'text-red-700' : 'text-amber-700'}`}>Total hoy: <strong>${new Intl.NumberFormat('es-AR').format(publicTotalToday)}</strong></p>
                  </div>
                </div>
              )}
              ...
              {showNext && (
                <div className={`rounded-2xl p-5 shadow-sm ${isToday ? 'bg-gradient-to-br from-rose-600 to-pink-500' : 'bg-white border border-gray-100'}`}>
                  ...
                  <p className={`font-semibold text-sm mt-0.5 ${isToday ? 'text-rose-100' : 'text-rose-600'}`}>{next.time} hs</p>
                </div>
              )}
              ...
              {showStats && (
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 space-y-3">
                  <div className="grid grid-cols-3 gap-2 text-center">
                    <div className="bg-blue-50 rounded-xl p-3"><p className="text-2xl font-black text-blue-800">{abonadas}</p><p className="text-[10px] font-bold text-blue-600 mt-0.5">Abonadas</p></div>
                    <div className="bg-green-50 rounded-xl p-3"><p className="text-2xl font-black text-green-800">{asistidas}</p><p className="text-[10px] font-bold text-green-600 mt-0.5">Asistidas</p></div>
                    <div className="bg-rose-50 rounded-xl p-3"><p className="text-2xl font-black text-rose-800">{restantes}</p><p className="text-[10px] font-bold text-rose-600 mt-0.5">Restantes</p></div>
                  </div>
                  <div className="space-y-1">
              ...
              {showSinMarcar && (
                <div className="bg-white rounded-2xl shadow-sm border border-amber-200 p-4 flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl bg-amber-50 flex items-center justify-center flex-shrink-0 text-xl">📋</div>
                  <div>
                    <p className="font-bold text-amber-800 text-sm">{sinMarcar.length} clase{sinMarcar.length>1?'s':''} sin marcar</p>
              ...
              {!hayAlgo && (
                checkingPayment ? (
                  ...
                  <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 text-center">
                    ...
                    <p className="font-bold text-gray-700 text-sm">¡Todo en orden!</p>
